...
triage: 221 traces  analyzed=221  failed=0  tier=white
  diagnosed 191  healthy 30  declined 0   abstention_rate=0.136

top clusters (ranked by n × mean_diagnostic_confidence — a triage ORDER, not measured impact):
   #     n  share  conf mode               headline signal            log-odds action             impact
   1    60  31.4%  0.99 hallucination      parametric_knowledge_score   +12.26 ground_or_abstain  28 fixed / 32 not
   2    56  29.3%  0.57 retrieval_failure  gold_recall_in_context       +12.63 add_gold_context   32 fixed / 0 not (24 unscored)
   3    32  16.8%  0.70 context_dilution   context_length_tokens        +13.72 rerank_gold_first  32 fixed / 0 not
```
...
**On production-shape traces (no ground truth) the shipped engine scores ~0.67, not
0.977.** That is the mode the tool actually runs in and it is now measured
...
silence (declined 0.000 → ~0.18). But it does not fail quiet entirely: precision on
...
> The raw transfer number is 0.593; ~0.674 is the figure after controlling for a mock
> artifact (the simulated model reads gold annotations that a real model never sees).
> §4.5 shows the decomposition and explains why the two must not be read additively.
...
tests/         229 tests (coherence, tiers, discriminators, causal chain, benchmark,
               ingest adapters, triage clustering, no-reference split, headless
               Streamlit app, shape-shortcut probe, ledger reconciliation, CLI seam)
```
