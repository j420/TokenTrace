...
**On production-shape traces (no ground truth, no gold flags) the shipped engine
scores 0.593, not 0.977.** That is the mode the tool actually runs in, it is now
measured (`tokentrace/eval/noref.py`, [§4.5](docs/REPORT.md)), and it fails *safe*:
precision stays 1.000 wherever it predicts, Top-3 only falls to 0.954, and what rises
is silence (declined 0.000 → 0.274). Retraining reference-free recovers 0.977.
